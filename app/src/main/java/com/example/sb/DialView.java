...
public class DialView extends View {
    ...
    private static int SELECTION_COUNT = 6;  // Total number of selections.
    private float mWidth;                    // Custom view width.
    private float mHeight;                   // Custom view height.
    private Paint mTextPaint,mTextPaint2;   // For text in the view.
    private Paint mDialPaint,mDailpaint2,mDialPaint3;   // For dial circle in the view.
    private float mRadius,mRadius2,mRadius3;                   // Radius of the dial.
    ...
    private void init() {
        ...
        mDailpaint2 = new Paint(Paint.ANTI_ALIAS_FLAG);
        mDailpaint2.setColor(Color.YELLOW);
        mDialPaint3 = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mWidth = w;
        mHeight = h;
        mRadius = (float) (Math.min(mWidth, mHeight) / 2 * 0.8);// size control
        mRadius2 =(float) (Math.min(mWidth, mHeight) / 2 * 0.75);// size control
        mRadius3 =(float) (Math.min(mWidth, mHeight) / 2 * 0.74);// size control
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(mWidth / 2, mHeight / 2, mRadius2, mDailpaint2);
        canvas.drawCircle(mWidth / 2, mHeight / 2, mRadius3, mDialPaint3);

        // Draw the text labels.
        final float labelRadius = mRadius + 40; //number distance from circle
        ...
        final float markerRadius = mRadius -60;//Circle Inner Diatnce
        float[] xyData = computeXYForPosition(mActiveSelection, markerRadius);
        float x = xyData[0];
        float y = xyData[1];
        canvas.drawCircle(x, y, 15, mTextPaint);//inner Circle size
    }
    ...
    private float[] computeXYForPosition(final int pos, final float radius) {
        ...
        Double startAngle = Math.PI * (9 / 2d);   // Angles are in radians.
        Double angle = startAngle + (pos * (Math.PI / 3));
